...
import { PurchasePagination } from '@/types/purchase';
...
watch(search, (value) => {
    router.get(purchase.index().url, { query: value }, { preserveState: true, replace: true });
});

const items = computed(() =>
    props.purchases.data.map((item) => ({
        id: item.id,
        code: item.code,
...
const columns = [
    ...
    columnHelper.accessor('supplierId', { header: 'Supplier' }),
    columnHelper.accessor('userId', { header: 'Cashier' }),
    columnHelper.accessor('status', {
        header: 'Status',
        cell: (info) => getStatusBadge(info.getValue() as string),
    }),
    columnHelper.accessor('subtotal', { header: 'Subtotal', cell: (info) => fmtIDR(info.getValue()) }),
    ...
    columnHelper.accessor('receivedAt', { header: 'Received Date', cell: (info) => fmtDate(info.getValue()) }),
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="rounded-lg bg-white shadow-md">
            <div class="space-y-4 p-6">
                    ...
                    <table class="min-w-full border-collapse bg-white">
                        ...
                        <tbody>
                            <tr v-if="tableData.getRowModel().rows.length === 0">
                                <td colspan="12" class="border-b px-4 py-8 text-center text-sm text-gray-500">No purchase data found</td>
                            </tr>
                ...
                <Pagination :pagination="props.purchases" class="p-2" />
            </div>
        </div>
    </AppLayout>
</template>
